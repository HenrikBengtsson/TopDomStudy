...
hic_bin_20180603 <- local({
  ...
  if (progress) {
    pb <- txtProgressBar(max = if (intra_only) length(known_chrs) else length(counts),
                         file = stderr())
    iter <- 0L
  }

  for (chr_a in known_chrs) {
    bins_a <- chr_bins[[chr_a]]
    ...
    for (chr_b in chrs_b) {
      ...
      bins_b <- chr_bins[[chr_b]]
  
      data_t <- data[data$chr_a == chr_a & data$chr_b == chr_b, ]
      
      if (nrow(data_t) > 0) {
        pos_a <- with(data_t, (start_a + end_a) / 2)
        pos_b <- with(data_t, (start_b + end_b) / 2)
	
        counts[[chr_a, chr_b]] %<-% {
          counts_t <- matrix(0L,
                             nrow = length(bins_a) - 1L,
                             ncol = length(bins_b) - 1L)
          attr(counts_t, "bins") <- list(chr_a = bins_a, chr_b = bins_b)
	  
          for (aa in seq_len(nrow(counts_t))) {
            keep <- which(bins_a[aa] <= pos_a & pos_a < bins_a[aa + 1L])
            if (length(keep) == 0L) next
            counts_t[aa, ] <- binCounts(pos_b[keep], bx = bins_b, right = FALSE)
